    ...
    <section id="home" className="relative min-h-[90vh] pt-16">
      <div className="container mx-auto px-4 py-4 md:py-8">
        <div className="flex flex-col items-center lg:items-start min-h-[calc(90vh-80px)] justify-center">
          <div className={`inline-block px-6 py-2 mb-4 rounded-full neo-blur border border-forest-light/30 text-white font-medium transform transition-all duration-700 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <motion.div 
              className="flex items-center gap-2" 
              animate={{ scale: [1, 1.03, 1] }} 
              transition={{ duration: 2, repeat: Infinity, repeatType: "loop" }}
            >
              <MountainSnow className="h-4 w-4 text-forest-light" />
