      ...
      <div className="container mx-auto px-4 bg-transparent">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            <Button 
              className="bg-gradient-to-r from-yellow-500 to-yellow-300 hover:from-yellow-600 hover:to-yellow-400 text-white border-0 shadow-lg shadow-yellow-600/30 transition-all duration-300 hover:shadow-yellow-600/50 animate-fade-in" 
            ...
            <Button 
              variant="outline" 
              className="md:hidden border border-yellow-500/30 text-offwhite hover:bg-yellow-500/20 animate-fade-in" 
          ...
          <nav className="flex flex-col space-y-4">
            {["Home", "Tour Packages", "Destinations", "Gallery", "About Us", "Contact"].map((item, index) => (
              <a 
                key={item} 
                href={`#${item.toLowerCase().replace(' ', '-')}`} 
                className={`text-sm font-medium text-offwhite hover:text-white transition-colors py-2 border-b border-yellow-500/20 transform translate-y-4 opacity-0 ${mobileMenuOpen ? 'animate-slide-up' : ''}`} 
                style={{animationDelay: `${index * 0.1}s`}} 
